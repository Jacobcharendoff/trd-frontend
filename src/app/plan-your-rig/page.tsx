...
const PEDAL_IMG_BASE = 'https://raw.githubusercontent.com/PedalPlayground/pedalplayground/master/public/images/pedals/';
...
function convertPPPedal(raw: PPRawPedal): Pedal {
  ...
  // PedalPlayground stores Width/Height in inches — convert to mm
  return {
    id,
    ...
    image: raw.Image,
    widthMm: Math.round(raw.Width * 25.4),
    heightMm: Math.round(raw.Height * 25.4),
  };
}
...
const featuredPedals: Pedal[] = [
  ...
  { id: 'strymon-mobius', name: 'Strymon Mobius', brand: 'Strymon', categoryId: 'modulation', color: '#0EA5E9', textColor: '#fff', shortName: 'Mobius', image: 'strymon-mobius.png', widthMm: 104, heightMm: 175 },
  ...
  { id: 'strymon-timeline', name: 'Strymon Timeline', brand: 'Strymon', categoryId: 'delay', color: '#0EA5E9', textColor: '#fff', shortName: 'Timeline', image: 'strymon-timeline.png', widthMm: 104, heightMm: 175 },
  ...
  { id: 'strymon-el-capistan', name: 'Strymon El Capistan', brand: 'Strymon', categoryId: 'delay', color: '#D97706', textColor: '#fff', shortName: 'El Capistan', image: 'strymon-elcap.png', widthMm: 104, heightMm: 175 },
  ...
  // Reverb
  { id: 'strymon-bigsky', name: 'Strymon BigSky', brand: 'Strymon', categoryId: 'reverb', color: '#0EA5E9', textColor: '#fff', shortName: 'BigSky', image: 'strymon-bigsky.png', widthMm: 104, heightMm: 175 },
  ...
  { id: 'strymon-flint', name: 'Strymon Flint', brand: 'Strymon', categoryId: 'reverb', color: '#DC2626', textColor: '#fff', shortName: 'Flint', image: 'strymon-flint.png', widthMm: 104, heightMm: 175 },
...
function PedalBlock({
  ...
  return (
    <button
      ...
      className={`group relative overflow-hidden hover:scale-[1.08] hover:brightness-110 transition-all duration-200 cursor-pointer ${
        visible ? 'opacity-100 scale-100' : 'opacity-0 scale-75'
      } ${!hasImage || !imgLoaded ? 'rounded-xl shadow-lg' : ''}`}
      style={{
        width: `${widthPx}px`,
        height: `${heightPx}px`,
        backgroundColor: hasImage && imgLoaded ? 'transparent' : pp.pedal.color,
    ...
    >
      ...
      {/* Pedal name overlay — only on fallback blocks, not on real images */}
      {hasImage && imgLoaded && (
        <div className="absolute bottom-0 left-0 right-0 px-1 pb-0.5 pt-2 pointer-events-none">
          <span className="text-[7px] sm:text-[9px] font-bold text-white text-center block leading-tight drop-shadow-[0_1px_3px_rgba(0,0,0,0.9)]">
            {pp.pedal.shortName}
          </span>
        </div>
      )}
      ...
      <div className={`absolute inset-0 ${!hasImage || !imgLoaded ? 'rounded-xl' : ''} bg-red-500/0 group-hover:bg-red-500/30 transition-colors duration-150 flex items-center justify-center`}>
        <svg viewBox="0 0 24 24" className="w-5 h-5 text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)] opacity-0 group-hover:opacity-100 transition-opacity duration-150" fill="none" stroke="currentColor" strokeWidth="2.5">
...
export default function PlanYourRig() {
  ...
  useEffect(() => {
    fetch('https://raw.githubusercontent.com/PedalPlayground/pedalplayground/master/public/data/pedals.json')
      .then((res) => res.json())
                  ...
                  {boardLayout && (
                    ...
                    >
                      ...
                      {placedPedals.length === 0 ? (
                        ...
                        const gapMm = 8; // 8mm gap between pedals
                        const gapPx = gapMm * pxPerMm;
                        const paddingMm = 12; // edge padding
                        const paddingPx = paddingMm * pxPerMm;
                        const usableWidthMm = boardLayout.widthMm - (paddingMm * 2);

                        // Arrange pedals into rows from bottom up (like a real pedalboard)
                        interface PedalPos { pp: PlacedPedal; x: number; y: number; w: number; h: number; }
                        const positions: PedalPos[] = [];
                        let currentRowWidthMm = 0;
                        const rows: { pedals: { pp: PlacedPedal; wMm: number; hMm: number }[]; maxHeightMm: number }[] = [];
                        let currentRow: { pp: PlacedPedal; wMm: number; hMm: number }[] = [];
                        let currentRowMaxH = 0;

                        for (const pp of signalChain) {
                          const wMm = pp.pedal.widthMm || 70;
                          const hMm = pp.pedal.heightMm || 120;
                          const neededWidth = currentRow.length > 0 ? wMm + gapMm : wMm;

                          if (currentRowWidthMm + neededWidth > usableWidthMm && currentRow.length > 0) {
                            // Start new row
                            rows.push({ pedals: currentRow, maxHeightMm: currentRowMaxH });
                            currentRow = [];
                            currentRowWidthMm = 0;
                            currentRowMaxH = 0;
                          }
                          currentRow.push({ pp, wMm, hMm });
                          currentRowWidthMm += (currentRow.length > 1 ? wMm + gapMm : wMm);
                          currentRowMaxH = Math.max(currentRowMaxH, hMm);
                        }
                        if (currentRow.length > 0) {
                          rows.push({ pedals: currentRow, maxHeightMm: currentRowMaxH });
                        }

                        // Position rows from bottom up, centered horizontally
                        let cursorYMm = boardLayout.heightMm - paddingMm; // start from bottom
                        for (const row of rows) {
                          const totalRowWidthMm = row.pedals.reduce((sum, p, i) => sum + p.wMm + (i > 0 ? gapMm : 0), 0);
                          let xMm = (boardLayout.widthMm - totalRowWidthMm) / 2; // center the row
                          const rowBottomY = cursorYMm;
                          for (const { pp, wMm, hMm } of row.pedals) {
                            const yMm = rowBottomY - hMm; // pedals sit on the row baseline
                            positions.push({
                              pp,
                              x: Math.round(xMm * pxPerMm),
                              y: Math.round(yMm * pxPerMm),
                              w: Math.round(wMm * pxPerMm),
                              h: Math.round(hMm * pxPerMm),
                            });
                            xMm += wMm + gapMm;
                          }
                          cursorYMm -= row.maxHeightMm + gapMm;
                        }
